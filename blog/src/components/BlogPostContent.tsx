                    ...
                    <nav className="space-y-1">
                      {tocItems.map((block) => (
                        <button
                          ...
                          className={`block text-sm text-left w-full py-1.5 px-3 rounded-lg transition-all border-l-2 ${activeSection === block.id
            ...
            <div className="col-span-1 lg:col-span-7">
              ...
              <div className="prose prose-slate prose-lg md:prose-xl max-w-none prose-headings:font-medium prose-headings:tracking-tight prose-a:text-indigo-600 hover:prose-a:text-indigo-500 prose-img:rounded-3xl prose-img:shadow-xl">
                {post.content_blocks.map((block: ContentBlock, idx: number) => {
                  ...
                  // NEW: Audio Player
                  if (block.type === 'audio_player' && block.src) {
                    return (
                      <div key={idx} className="not-prose">
                        <AudioPlayer
                          src={block.src}
                          title={block.title || 'Audio Clip'}
                          duration={block.duration}
                          type={block.audio_type}
                        />
                      </div>
                    );
                  }
                  ...
                  if (block.type === 'product_showcase' && block.image) {
                    return (
                      <div key={idx} className="not-prose">
                        <ProductShowcase
                          title={block.title || ''}
                  ...
                  // NEW: Glossary Term
                  if (block.type === 'glossary_term' && block.term) {
                    return (
                      <div key={idx} className="not-prose">
                        <GlossaryTerm
                          term={block.term}
